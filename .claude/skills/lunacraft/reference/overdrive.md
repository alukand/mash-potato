...
- **Scroll-driven animations** (`animation-timeline: scroll()`): CSS-only, no JS. Parallax, progress bars, reveal sequences all driven by scroll position. (All evergreen browsers as of 2026; still wrap in `@supports (animation-timeline: scroll())` and provide a static fallback for older targets)
...
### Named Ambition Patterns

Specific high-impact techniques built on the technologies above. Each describes a concrete result, not just a technology.

- **Dynamic Island**: A morphing container that changes shape and content based on application state. A button that expands into a notification bar, then reshapes into a mini-player. Use View Transitions or FLIP animations for the morph; the container's `border-radius` and dimensions animate simultaneously.
- **Liquid Glass Refraction**: Beyond basic `backdrop-blur`. Add a 1px inner border at `white/10` opacity + an `inset` box shadow for physical glass-edge simulation. The blur creates the refraction; the border and shadow create the physical object. Distinct from generic glassmorphism because it simulates a real glass edge, not a frosted overlay.
- **Scroll-Driven Section Transitions**: CSS `animation-timeline: scroll()` to morph between section themes as the user scrolls. Background color, typography scale, and element positions all interpolate based on scroll position. Progressive enhancement: wrap in `@supports (animation-timeline: scroll())`.
- **GPU-Accelerated Data Canvas**: Canvas or WebGL rendering for datasets too large for DOM. 100k+ row tables, force-directed graphs, real-time streaming data. The DOM becomes a thin interaction overlay on top of GPU-rendered content.
- **Shared Element Morph**: View Transitions API to morph a list item into a detail view, or a button into a dialog. The element maintains visual identity across page states. Use `view-transition-name` on the source and destination elements. Progressive enhancement: wrap in `if (document.startViewTransition)`.
