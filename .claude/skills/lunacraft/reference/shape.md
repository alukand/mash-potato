...
**Output**: A design brief that can be handed off to /lunacraft craft, or directly to /lunacraft for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Discovery includes at least one user-answer round unless project brief, DESIGN.md, or an already-confirmed brief directly answers the needed inputs. With a sparse prompt, do **not** synthesize a complete brief for confirmation on the first response.

- Use the harness's structured question tool when one exists. Otherwise, ask directly in chat and stop.
- Ask **2-3 questions per round**, then wait for answers.
- Treat project brief and DESIGN.md as anchors; they reduce repeated questions but do **not** replace shape for craft. Shape is task-specific.
...
**Assert-then-confirm, not menu-with-escape.** When project brief and the user's prompt make one option obvious, name it and ask the user to confirm or override. Don't enumerate "Restrained / Committed / Or something else?" as a real choice; "This reads as Restrained, confirm?" beats a four-option menu when the answer is already clear.

### Purpose & Context
- What is this feature for? What problem does it solve?
- Who specifically will use it? (Not "users"; be specific: role, context, frequency)
- How does this audience decide or work? (What do they need to see, feel, or verify before they act — and in what order? See SKILL.md § Audience fit.)
...
Force a visual decision on three fronts. Skip anything project brief or DESIGN.md already answers; ask only what's missing.
...
Scope answers are task-scoped. Don't write them to project brief or DESIGN.md; carry them through the design brief only.
...
**3. Design Direction**
Color strategy (Restrained / Committed / Full palette / Drenched) + the theme scene sentence + 2–3 named anchor references. Reference project brief and DESIGN.md where they already answer, and note any per-surface overrides.
...
Based on the brief, list which LunaCraft reference files would be most valuable during implementation (e.g., spatial-design.md for complex layouts, motion-design.md for animated features, interaction-design.md for form-heavy features).
...
Once confirmed, the brief is complete. The user can now hand it to /lunacraft, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /lunacraft craft instead, which runs this command internally.)
