...
- The user just ran `/lunacraft teach` and needs the visual side documented.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. `/lunacraft document --seed` forces seed mode regardless of code presence.
...
- **Creative North Star**: a single named metaphor for the whole system ("The Editorial Sanctuary", "The Golden State Curator", "The Lab Notebook"). Offer 2-3 options that honor project brief's brand personality.
...
Quote a line from project brief when possible so the user sees their own strategic language carry forward.
...
[2-3 paragraph holistic description: personality, density, aesthetic philosophy. Start from the North Star and work outward. State what this system explicitly rejects (pulled from project brief's anti-references). End with a short **Key Characteristics:** bullet list.]
...
Concrete, forceful guardrails. Lead each with "Do" or "Don't". Be specific: include exact colors, pixel values, and named anti-patterns the user mentioned in project brief. **Every anti-reference in project brief should show up here as a "Don't" with the same language**, so the visual spec carries the strategic line through. Quote project brief directly where possible: if project brief says *"avoid dark mode with purple gradients, neon accents, glassmorphism"*, the Don'ts here should repeat that by name.
...
### Step 4b: Write .lunacraft/design.json sidecar (extensions only)

The frontmatter owns token primitives (colors, typography, rounded, spacing, components). The sidecar at `.lunacraft/design.json` carries **what Stitch's schema can't hold**: tonal ramps per color, shadow/elevation tokens, motion tokens, breakpoints, full component HTML/CSS snippets (the panel renders these into a shadow DOM), and narrative (north star, rules, do's/don'ts). It extends the frontmatter, it doesn't duplicate it.

Regenerate the sidecar whenever you regenerate root `DESIGN.md`. If the user only asks to refresh the sidecar (e.g., from the live panel's stale-hint), preserve `DESIGN.md` and write only `.lunacraft/design.json`.
...
- **Signature components (include if distinctive):** hero CTA, featured card, filter pill, any custom pattern the user mentioned as important in project brief.
- **Skip the rest.** Utility components, form building blocks, wrapper layouts: not worth documenting unless visually distinctive.

If the project has **no component library yet** (bare landing page, new project), synthesize canonical primitives from the tokens using best-practice defaults consistent with the DESIGN.md's rules. Every `.lunacraft/design.json` has *something* to render, even on day zero.
...
2. Mention that `.lunacraft/design.json` was also written alongside; the live panel will now render this project's actual button/input/nav primitives instead of generic approximations.
...
4. **Refresh the session cache.** Run `node $HOME/.claude/skills/lunacraft/scripts/load-context.mjs` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `/lunacraft document` once there's code, to capture the real tokens and components. OK?"
...
```markdown
<!-- SEED: re-run /lunacraft document once there's code to capture the actual tokens and components. -->
```
...
- **Do's and Don'ts**: carry project brief's anti-references directly plus the anti-reference named in Q5.

Seed mode writes a minimal frontmatter with `name` and `description` only; no colors, typography, rounded, spacing, or components yet. Real tokens land on the next Scan-mode run. Skip the `.lunacraft/design.json` sidecar in seed mode for the same reason: nothing to render.
...
2. Tell the user: "Re-run `/lunacraft document` once you have some code. That pass will extract real tokens and generate the sidecar."
3. Run `node $HOME/.claude/skills/lunacraft/scripts/load-context.mjs` once so the seed lands in conversation for the rest of the session.
...
- **Cite project brief anti-references by name** in the Do's and Don'ts section. If project brief lists "SaaS landing-page clichés" or "generic AI tool marketing" as anti-references, the DESIGN.md Don'ts should repeat those phrases verbatim so the visual spec enforces the strategic line.
...
- **Be forceful**. The voice of a design director. "Prohibited", "forbidden", "never", "always", not "consider", "might", "prefer". Match project brief's tone.
...
- **Reference project brief**. The anti-references section of project brief should directly inform the Do's and Don'ts section here. Quote or paraphrase.
...
- Don't duplicate content from project brief. DESIGN.md is strictly visual.
