...
Chat is overhead. No recap, no tutorial output, no pasting project brief / DESIGN / TECHNICAL bodies. Spend tokens on tools and edits; on failure, one or two short sentences.

## Start

```bash
node $HOME/.claude/skills/lunacraft/scripts/live.mjs
```

Output JSON: `{ ok, serverPort, serverToken, pageFiles, hasProjectBrief, projectBrief, projectBriefPath, hasProduct, product, productPath, hasDesign, design, designPath, hasTechnical, technical, technicalPath, migrated }`. `pageFiles` is the list of HTML entries the live script was injected into. **`load-context.mjs` CLI** (run separately) also emits `hasSeoKeywords` and `seoKeywordsPath` when `SEO-KEYWORDS.md` exists; it does not embed the map body in JSON. Keep project brief, DESIGN.md, and TECHNICAL.md in mind for variant generation; **DESIGN.md wins on visual decisions; project brief wins on strategic/voice decisions; TECHNICAL.md constrains implementation choices.** When DESIGN.md is missing, identity is **not** absent; extract it from CSS variables, computed styles, and sibling components on the page (see Step 4 Phase A). Identity preservation is the default; departure from existing identity requires an explicit trigger from project brief anti-references or the user's freeform prompt. If `migrated: true`, the loader auto-renamed legacy `.impeccable.md` or `.lunacraft.md` to `PROJECT-BRIEF.md`; mention this once and suggest `/lunacraft document` for the matching DESIGN.md.

`serverPort` and `serverToken` belong to the small **LunaCraft live helper** HTTP server (serves `/live.js`, SSE, and `/poll`). That port is **not** your dev server and is usually not the URL you open to view the app. The browser page is whatever origin serves one of the `pageFiles` entries (Vite / Next / Bun / tunnel / LAN hostname).
...
```
LOOP:
  node $HOME/.claude/skills/lunacraft/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
The live helper persists an append-only journal under `.lunacraft/live/sessions/`. Browser checkpoints are advisory but durable; the journal is canonical. This is local durable recovery state, not project source.

Use these commands when the chat was interrupted, polling was missed, the helper restarted, or the browser reloaded:

```bash
node $HOME/.claude/skills/lunacraft/scripts/live-status.mjs
node $HOME/.claude/skills/lunacraft/scripts/live-resume.mjs --id SESSION_ID
node $HOME/.claude/skills/lunacraft/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node $HOME/.claude/skills/lunacraft/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
**Departure mode**: the existing identity is rejected. Variants propose alternatives consistent with project brief voice. Trigger only when at least one is true:

- project brief anti-references explicitly call out the current surface ("the current `index.html` is itself an example"; "diffuse away from this"; "the page on screen is the failure"). Generic anti-references that describe what to avoid in general do **not** trigger departure mode; only ones that point at *this* surface specifically.
...
**Departure mode.** Each variant anchors to a different **aesthetic direction**, derived from the brand's stated voice and register in project brief. Do NOT pick from a fixed catalog of lane categories. The right three directions for this brand are not the same as the right three for another brand, and picking from a list is itself the training-data reflex (the model selects "Swiss-grid, Terminal, Industrial-signage" every time because those are the furthest-from-editorial items in any enumerated list).

Instead, work from the brand:

1. Read project brief's Brand Personality words. What physical, spatial, or material experiences would embody those words if design were not involved? (A personality described as "specific, earned, unmistakable" evokes a hand-stamped letter, a numbered print, a watchmaker's loupe. A personality described as "restless, loud, unfiltered" evokes a concert poster, a spray-painted wall, a megaphone.)
...
- `animate`: different motion vocabulary (cascade stagger / clip wipe / scale-and-focus / morph / parallax). Not three staggered fades. **Live explores variants** — when committing motion to a shipped surface, follow [animate.md](animate.md) checklist (non-destructive, LCP guard, register fork).
...
When the prompt and project brief anti-references conflict (the prompt asks for X, the anti-references ban X), the anti-references win; they describe the brand's standing position, the prompt is one moment.
...
```bash
node $HOME/.claude/skills/lunacraft/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node $HOME/.claude/skills/lunacraft/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node $HOME/.claude/skills/lunacraft/scripts/live-server.mjs stop
```

Stops the HTTP server and runs `live-inject.mjs --remove` to strip `localhost:…/live.js` from the HTML entry. To stop the server but keep the inject tag (for a quick restart), use `stop --keep-inject`. `.lunacraft/live/config.json` persists as project config for future sessions.
...
If `live.mjs` outputs `{ ok: false, error: "config_missing" | "config_invalid", path }`, write the live config at the reported path. By default this is `.lunacraft/live/config.json`.
...
```bash
node $HOME/.claude/skills/lunacraft/scripts/detect-csp.mjs
```
...
- **`null`**: no CSP; skip to writing `.lunacraft/live/config.json` with `cspChecked: true`.
...
```ts
// Dev-only allowance so LunaCraft live mode can load. Guarded by NODE_ENV.
...
```ts
// Dev-only allowance so LunaCraft live mode can load.
const __impeccableLiveDev =
  process.env.NODE_ENV === "development" ? " http://localhost:8400" : "";
```
...
If a user says "no" to the CSP patch at setup time and later complains that live doesn't work: their dev CSP blocks `http://localhost:8400`. Fix: delete `cspChecked` from `.lunacraft/live/config.json` and re-run `live.mjs`: setup will ask again.
