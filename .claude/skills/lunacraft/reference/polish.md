...
Final precision QA pass. Pixel alignment, spacing consistency, interaction state completeness, design-system compliance, responsive checks, code cleanup, performance. Nothing new gets added here — everything that exists gets refined.

This is the design-lock phase. It assumes the design is structurally sound (improve handles structural problems). Polish aligns spacing, states, and tokens — it does not add motion (that's animate's job after polish).
...
4. **Pull in any prior critique** (optional signal): If `/lunacraft critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path or URL, then:
   ```bash
   $slug = node "$HOME/.claude/skills/lunacraft/scripts/critique-storage.mjs" slug "<resolved>"
   node "$HOME/.claude/skills/lunacraft/scripts/critique-storage.mjs" latest $slug
   ```
...
**CRITICAL**: Polish is the design-lock step, not the first step in the pipeline. Don't polish work that's not functionally complete.
...
- Polish before running improve; structural problems need improve, not polish
...
- Add animation or motion (that's **animate**'s job after polish; polish only verifies existing motion is smooth)
...
> **Polish complete.** Design is locked: alignment, spacing, states, and tokens aligned.
>
> Next step: run **animate** for motion, or ship if motion is already complete.
