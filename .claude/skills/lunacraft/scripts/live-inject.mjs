 ...
 *
 * On first live run, the agent generates `.lunacraft/live/config.json`
...
import { resolveLiveConfigPath } from './lunacraft-paths.mjs';
...
Reads configuration from .lunacraft/live/config.json.

Modes:
  --port PORT   Insert script tag pointing at http://localhost:PORT/live.js
  --remove      Remove the script tag (if present)
  --check       Print whether .lunacraft/live/config.json exists and its content
