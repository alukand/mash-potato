 ...
 *   1. Check .lunacraft/live/config.json (returns config_missing if first-ever run)
 ...
 *   4. Read project brief / DESIGN.md / TECHNICAL.md for project context
...
Prepare everything for live variant mode in a single command:
  - Checks .lunacraft/live/config.json (required, created once per project)
  - Starts (or reuses) the live server in the background
  - Injects the browser script tag
  - Reads project brief / DESIGN.md / TECHNICAL.md for project context
...
The agent should then:
  ...
  // 4. Load project brief + DESIGN.md + TECHNICAL.md context
  ...
  console.log(JSON.stringify({
    ...
    configDrift: drift,
    hasProjectBrief: ctx.hasProjectBrief,
    projectBrief: ctx.projectBrief,
    projectBriefPath: ctx.projectBriefPath,
    ...
    hasTechnical: ctx.hasTechnical,
    technical: ctx.technical,
    technicalPath: ctx.technicalPath,
