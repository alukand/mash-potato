...
export const LUNACRAFT_DIR = '.lunacraft';
...
export function getLunaCraftDir(cwd = process.cwd()) {
  return path.join(cwd, LUNACRAFT_DIR);
}

export function getDesignSidecarPath(cwd = process.cwd()) {
  return path.join(getLunaCraftDir(cwd), 'design.json');
}

export function getDesignSidecarCandidates(cwd = process.cwd(), contextDir = cwd) {
  const candidates = [
    getDesignSidecarPath(cwd),
    path.join(getImpeccableDir(cwd), 'design.json'),
...
export function getLiveDir(cwd = process.cwd()) {
  return path.join(getLunaCraftDir(cwd), LIVE_DIR);
}
...
export function resolveLiveConfigPath({ cwd = process.cwd(), scriptsDir, env = process.env } = {}) {
  const configuredEnv = env.LUNACRAFT_LIVE_CONFIG || env.IMPECCABLE_LIVE_CONFIG;
  if (configuredEnv && configuredEnv.trim()) {
...
export function getCritiqueDir(cwd = process.cwd()) {
  return path.join(getLunaCraftDir(cwd), CRITIQUE_DIR);
}
