 ...
 *
 * Usage:
 *   npx lunacraft poll                         # Block until browser event, print JSON
 *   npx lunacraft poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   npx lunacraft poll --reply <id> done       # Reply "done" to event <id>
 *   npx lunacraft poll --reply <id> error "msg" # Reply with error
 */
...
import { readLiveServerInfo } from './lunacraft-paths.mjs';
...
function readServerInfo() {
  ...
  if (!record) {
    console.error('No running live server found. Start one with: npx lunacraft live');
    process.exit(1);
  }
  return record.info;
}
...
Options:
  ...
  // Reply mode: npx lunacraft poll --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: npx lunacraft poll --reply <id> <status> [--file path] [message]');
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        console.error('Live server not running. Start one with: npx lunacraft live');
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error('Try restarting: npx lunacraft live stop && npx lunacraft live');
        process.exit(1);
      }
