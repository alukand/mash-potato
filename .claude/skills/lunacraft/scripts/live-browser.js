/**
 * LunaCraft Live Variant Mode — Browser Script
 *
    ...
    evtSource.onmessage = (e) => {
      ...
      switch (msg.type) {
        case 'connected':
          ...
          if (!hasProjectContext) showToast('No project brief found. Variants will be brand-agnostic. Run /lunacraft teach or initialize project context to generate one.', 7000);
...
void main() {
  ...
  // LunaCraft logo mark — matches the site-header SVG (rounded square + "/").
  ...
  // ---------------------------------------------------------------------------
  // Design System Panel — visualizes the project's .lunacraft/design.json sidecar
  // ---------------------------------------------------------------------------
  ...
  let designState = {
    ...
    sidecar: null,          // .lunacraft/design.json v2 payload (extensions + components + narrative)
    hasMd: false,
    hasSidecar: false,
  ...
  function renderDesignBody() {
    ...
    if (designState.present === false) {
      ...
      empty.innerHTML = `<strong>No DESIGN.md yet</strong>Create one by running <code>/lunacraft document</code> in your terminal, then re-open this panel.`;
  ...
  function renderStaleHint() {
    ...
    box.innerHTML = `
      ...
      <span class="stale-text"><strong>DESIGN.md is newer than .lunacraft/design.json.</strong> Run <code>/lunacraft document</code> to refresh the sidecar.</span>
    `;
    return box;
  }
  ...
  function cssSafe(v) {
    ...
    // .lunacraft/design.json values rendered into inline style strings.
