...
import {
  ...
  writeLiveServerInfo,
} from './lunacraft-paths.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
// Project brief / DESIGN.md / TECHNICAL.md live wherever load-context.mjs resolves. The generated
// DESIGN sidecar is project-local at .lunacraft/design.json, with legacy
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    path.join(__dirname, '..', 'node_modules', 'impeccable', 'cli', 'engine', 'detect-antipatterns-browser.js'),
...
function hasProjectContext() {
  // The project brief carries brand voice / anti-references — that's what
  // determines whether variants are brand-aware. DESIGN.md (visual tokens) and
  // TECHNICAL.md (implementation constraints) are separate concerns.
  return loadContext(process.cwd()).hasProjectBrief;
}
...
function createRequestHandler({ detectScript, sessionPath, livePath }) {
  return (req, res) => {
    ...
    //   /design-system.json    returns both parsed DESIGN.md and .lunacraft/design.json
    ...
    //                          - sidecar: .lunacraft/design.json contents when present.
    ...
    if (p === '/design-system.json' || p === '/design-system/raw') {
      ...
      if (jsonStat) {
        try {
          ...
          response.sidecarError = 'Failed to parse .lunacraft/design.json: ' + err.message;
        }
      }
...
httpServer.listen(state.port, '127.0.0.1', () => {
  ...
  console.log(`\nLunaCraft live server running on ${url}`);
